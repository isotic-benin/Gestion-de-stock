...
if (!hasPermission($conn, 'dashboard', 'read')) {
...
// Définir les permissions pour l'affichage conditionnel des sections
$can_view_produits = hasPermission($conn, 'produits', 'read');
$can_view_transferts = hasPermission($conn, 'transferts_stock', 'read');
$can_view_ventes = hasPermission($conn, 'ventes', 'read');
$can_view_clients = hasPermission($conn, 'clients', 'read');
$can_view_magasins = hasPermission($conn, 'magasins', 'read');
$can_view_fournisseurs = hasPermission($conn, 'fournisseurs', 'read');

// Récupérer la devise depuis les infos entreprise
$devise = $company_info['devise'] ?? 'XOF';

// 1. Produits en alerte de stock (seulement si l'utilisateur peut voir les produits)
$produits_alerte = [];
if ($can_view_produits) {
...
// 2. Transferts de stock non confirmés (seulement si l'utilisateur peut voir les transferts)
$transferts_non_confirmes = [];
if ($can_view_transferts) {
...
// 3. Produits les plus vendus (30 derniers jours) - seulement si l'utilisateur peut voir les ventes
...
// 4. Meilleurs clients (30 derniers jours) - seulement si l'utilisateur peut voir les clients
$meilleurs_clients = [];
if ($can_view_clients) {
...
// 5. Magasins avec le plus de chiffre d'affaires (30 derniers jours) - seulement si l'utilisateur peut voir les magasins
$magasins_ca = [];
if ($can_view_magasins) {
...
// 6. Fournisseurs dont les produits sont les plus vendus (30 derniers jours) - seulement si l'utilisateur peut voir les fournisseurs
$fournisseurs_top = [];
if ($can_view_fournisseurs) {
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6 pb-20">
            <div class="container mx-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    ...
                    <?php if ($can_view_transferts): ?>
                        <div class="bg-white rounded-lg shadow-md p-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-gray-600 text-sm">Transferts en Attente</p>
                                    <p class="text-2xl font-bold text-orange-600">
                                        <?php echo count($transferts_non_confirmes); ?>
                                    </p>
                                </div>
                                    ...
                                    <table class="min-w-full">
                                        ...
                                        <tbody class="divide-y divide-gray-200">
                                            <?php foreach ($transferts_non_confirmes as $transfert): ?>
                                                <tr class="hover:bg-gray-50">
                                                    <td class="px-4 py-2 text-sm">
                                                        <?php echo htmlspecialchars($transfert['produit_nom']); ?>
                                                    </td>
                                                    <td class="px-4 py-2 text-sm">
                                                    ...
                                                    <td class="px-4 py-2 text-sm">
                                                        <?php echo number_format($transfert['quantite'], 2, ',', ' '); ?>
                                                    </td>
                                                    <td class="px-4 py-2 text-sm">
                                                        <?php echo date('d/m/Y', strtotime($transfert['date_demande'])); ?>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
                                ...
                                <table class="min-w-full">
                                    <thead>
                                        <tr class="bg-gray-50">
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">
                                                Fournisseur</th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Contact
                                            </th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Produits
                                                Vendus</th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Qté
                                                Totale</th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Chiffre
                                                d'Affaires</th>
                                        </tr>
