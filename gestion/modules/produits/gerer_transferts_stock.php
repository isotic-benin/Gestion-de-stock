...
// Vérifier les permissions d'accès au module transferts de stock (lecture)
if (!hasPermission($conn, 'transferts_stock', 'read')) {
...
$can_create = hasPermission($conn, 'transferts_stock', 'create');
$can_update = hasPermission($conn, 'transferts_stock', 'update');
$can_delete = hasPermission($conn, 'transferts_stock', 'delete');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['add_transfert']) || isset($_POST['edit_transfert']))) {
    ...
    if (isset($_POST['add_transfert']) && !$can_create) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de demander un transfert.</div>';
        goto end_form_processing_transfert;
    }
    if (isset($_POST['edit_transfert']) && !$can_update) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de modifier un transfert.</div>';
        goto end_form_processing_transfert;
    }
        ...
        if ($stmt_check_stock = mysqli_prepare($conn, $sql_check_stock)) {
            ...
            if ($quantite > $adjusted_stock_disponible) {
                ...
                if (isset($_POST['add_transfert'])) {
                    ...
                    // EMPÊCHER LA MODIFICATION SI DÉJÀ VALIDÉ/REJETÉ
                    if ($old_statut != 'en_attente') {
                        $message = '<div class="alert alert-error">Impossible de modifier un transfert qui n\'est plus en attente.</div>';
                        goto end_form_processing_transfert;
                    }
...
end_form_processing_transfert:
;

// Gérer la suppression d'un transfert
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_transfert'])) {
    if (!$can_delete) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de supprimer un transfert.</div>';
        goto end_delete_transfert;
    }
    $transfert_id = sanitize_input($_POST['transfert_id']);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['confirm_transfert']) || isset($_POST['reject_transfert']) || isset($_POST['cancel_transfert']))) {
    if (!$can_update) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de valider ou rejeter un transfert.</div>';
        goto end_action;
    }
                                    ...
                                    <?php foreach ($transferts as $transfert): ?>
                                        <tr>
                                            ...
                                            <td class="border px-4 py-2 action-buttons flex flex-col md:flex-row gap-2">
                                                <?php if ($can_update && $transfert['statut'] == 'en_attente'): ?>
                                                    <button
                                                ...
                                                <?php if ($transfert['statut'] == 'en_attente' && $can_update): ?>
                                                    <button
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Afficher le stock disponible du produit sélectionné, pré-remplir le magasin source et filtrer le magasin destination
        produitIdSelect.addEventListener('change', function () {
            const selectedOption = this.options[this.selectedIndex];
            const stock = selectedOption.dataset.stock;
            const magasinId = selectedOption.dataset.magasin_id || selectedOption.dataset.magasinId; // Gérer les deux cas de naming si besoin

            // 1. Affichage du stock
            if (stock !== undefined) {
            ...
            // 2. Gestion du magasin source et destination
            if (magasinId) {
                // Sélectionner le magasin source
                magasinSourceIdSelect.value = magasinId;
                magasinSourceIdSelect.classList.add('bg-gray-200');
                magasinSourceIdSelect.style.pointerEvents = 'none';

                // Filtrer le magasin destination pour exclure le magasin source
                for (let i = 0; i < magasinDestinationIdSelect.options.length; i++) {
                    const option = magasinDestinationIdSelect.options[i];
                    if (option.value === magasinId) {
                        option.style.display = 'none'; // Masquer
                        // Si l'option masquée était sélectionnée, on désélectionne
                        if (magasinDestinationIdSelect.value === magasinId) {
                            magasinDestinationIdSelect.value = '';
                        }
                    } else {
                ...
                // Reset si pas de magasin associé
                magasinSourceIdSelect.value = '';
                magasinSourceIdSelect.classList.remove('bg-gray-200');
                magasinSourceIdSelect.style.pointerEvents = 'auto';

                // Réafficher toutes les destinations
                for (let i = 0; i < magasinDestinationIdSelect.options.length; i++) {
                    magasinDestinationIdSelect.options[i].style.display = '';
                }
            }
        });

        // Réactiver les champs disabled au moment du submit pour qu'ils soient envoyés
        transfertForm.addEventListener('submit', function () {
            magasinSourceIdSelect.style.pointerEvents = 'auto';
            // Note: Si on avait utilisé disabled=true, il aurait fallu le passer à false ici.
        });
