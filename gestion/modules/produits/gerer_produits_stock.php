...
if (!hasPermission($conn, 'produits', 'read')) {
...
$can_create = hasPermission($conn, 'produits', 'create');
$can_update = hasPermission($conn, 'produits', 'update');
$can_delete = hasPermission($conn, 'produits', 'delete');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['add_produit']) || isset($_POST['edit_produit']))) {
    ...
    if (isset($_POST['add_produit']) && !$can_create) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de créer un produit.</div>';
        goto end_form_processing;
    }
    if (isset($_POST['edit_produit']) && !$can_update) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de modifier un produit.</div>';
        goto end_form_processing;
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_produit'])) {
    if (!$can_delete) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de supprimer un produit.</div>';
        goto skip_delete_product;
    }
