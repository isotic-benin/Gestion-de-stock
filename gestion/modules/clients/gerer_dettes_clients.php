...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['payer_dette'])) {
    ...
    if (!$can_update) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission d\'enregistrer des paiements.</div>';
    } else {
    ...
    // Gérer le paiement d'une dette client via Compte Épargne
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['payer_dette_epargne'])) {
        // Vérifier la permission de modification
        if (!$can_update) {
            $message = '<div class="alert alert-error">Vous n\'avez pas la permission d\'effectuer des paiements via compte épargne.</div>';
        } else {
            $dette_id = sanitize_input($_POST['dette_id_paiement_epargne']);
            $montant_paiement = (float) sanitize_input($_POST['montant_paiement_epargne']);

            // Récupérer la dette et le compte épargne du client
            $sql_info = "SELECT dc.montant_restant, dc.client_id, ce.id AS compte_epargne_id, ce.solde AS solde_epargne 
                 FROM dettes_clients dc 
                 JOIN comptes_epargne ce ON dc.client_id = ce.client_id 
                 WHERE dc.id = ?";

            if ($stmt_info = mysqli_prepare($conn, $sql_info)) {
                mysqli_stmt_bind_param($stmt_info, "i", $dette_id);
                mysqli_stmt_execute($stmt_info);
                $res_info = mysqli_stmt_get_result($stmt_info);

                if ($row_info = mysqli_fetch_assoc($res_info)) {
                    $montant_restant_dette = $row_info['montant_restant'];
                    $solde_epargne = $row_info['solde_epargne'];
                    $compte_epargne_id = $row_info['compte_epargne_id'];

                    if ($montant_paiement <= 0) {
                        $message = '<div class="alert alert-error">Le montant du paiement doit être positif.</div>';
                    } elseif ($montant_paiement > $montant_restant_dette) {
                        $message = '<div class="alert alert-error">Le montant du paiement dépasse le montant restant dû de la dette.</div>';
                    } elseif ($montant_paiement > $solde_epargne) {
                        $message = '<div class="alert alert-error">Solde épargne insuffisant pour effectuer ce paiement.</div>';
                    } else {
                        // Tout est OK, on procède à la transaction
                        // 1. Débiter le compte épargne
                        // 2. Enregistrer la transaction épargne
                        // 3. Mettre à jour la dette

                        mysqli_begin_transaction($conn);
                        try {
                            // Update compte épargne
                            $new_solde = $solde_epargne - $montant_paiement;
                            $sql_update_compte = "UPDATE comptes_epargne SET solde = ? WHERE id = ?";
                            $stmt_uc = mysqli_prepare($conn, $sql_update_compte);
                            mysqli_stmt_bind_param($stmt_uc, "di", $new_solde, $compte_epargne_id);
                            mysqli_stmt_execute($stmt_uc);
                            mysqli_stmt_close($stmt_uc);

                            // Insert transaction épargne
                            $desc_trans = "Paiement dette #" . $dette_id;
                            $type_trans = "retrait";
                            $sql_insert_trans = "INSERT INTO transactions_epargne (compte_epargne_id, type_transaction, montant, description) VALUES (?, ?, ?, ?)";
                            $stmt_it = mysqli_prepare($conn, $sql_insert_trans);
                            mysqli_stmt_bind_param($stmt_it, "isds", $compte_epargne_id, $type_trans, $montant_paiement, $desc_trans);
                            mysqli_stmt_execute($stmt_it);
                            mysqli_stmt_close($stmt_it);

                            // Update dette
                            $new_montant_restant_dette = $montant_restant_dette - $montant_paiement;
                            $new_statut_dette = ($new_montant_restant_dette <= 0) ? 'payee' : 'en_cours';
                            $sql_update_dette = "UPDATE dettes_clients SET montant_restant = ?, statut = ? WHERE id = ?";
                            $stmt_ud = mysqli_prepare($conn, $sql_update_dette);
                            mysqli_stmt_bind_param($stmt_ud, "dsi", $new_montant_restant_dette, $new_statut_dette, $dette_id);
                            mysqli_stmt_execute($stmt_ud);
                            mysqli_stmt_close($stmt_ud);

                            mysqli_commit($conn);
                            $message = '<div class="alert alert-success">Paiement via épargne effectué avec succès !</div>';
                    ...
                    $message = '<div class="alert alert-error">Impossible de récupérer les informations du compte épargne ou de la dette.</div>';
                }
...
$sql = "SELECT dc.id, dc.client_id, dc.montant_initial, dc.montant_restant, dc.date_creation, dc.date_limite_paiement, dc.description, dc.statut, c.nom AS client_nom, c.prenom AS client_prenom, ce.id AS compte_epargne_id, ce.solde AS solde_epargne
        FROM dettes_clients dc
        JOIN clients c ON dc.client_id = c.id
        LEFT JOIN comptes_epargne ce ON c.id = ce.client_id"
                                        ...
                                        <tr>
                                            ...
                                            <td class="border px-4 py-2 action-buttons flex flex-wrap gap-2">
                                                ...
                                                <?php if ($dette['statut'] == 'en_cours' && $dette['montant_restant'] > 0 && $can_update): ?>
                                                    ...
                                                    <!-- Bouton Paiement via Épargne -->
                                                    <?php if (!empty($dette['compte_epargne_id']) && $dette['solde_epargne'] > 0): ?>
                                                        <button
                                                            class="btn btn-info payer-dette-epargne-btn bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded-md text-sm"
                                                            data-id="<?php echo htmlspecialchars($dette['id']); ?>"
                                                            data-client_nom="<?php echo htmlspecialchars($dette['client_nom'] . ' ' . $dette['client_prenom']); ?>"
                                                            data-montant_restant="<?php echo htmlspecialchars($dette['montant_restant']); ?>"
                                                            data-solde_epargne="<?php echo htmlspecialchars($dette['solde_epargne']); ?>">
...
<!-- Modal de paiement de dette via Épargne -->
<div id="payerDetteEpargneModal"
    class="modal hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
    <div class="modal-content bg-white p-6 rounded-lg shadow-xl max-w-md w-11/12 mx-auto">
        <span
            class="modal-close-button absolute top-3 right-3 text-gray-600 hover:text-gray-900 text-2xl cursor-pointer">&times;</span>
        <h2 class="text-2xl font-bold text-gray-800 mb-4 text-blue-600"><i class="fas fa-piggy-bank mr-2"></i>Payer via
            Épargne</h2>

        <div class="bg-blue-50 p-4 rounded-lg mb-4">
            <p class="text-gray-700 mb-2">Client: <span id="paiementEpargneClientNom" class="font-semibold"></span></p>
            <div class="flex justify-between items-center mb-1">
                <span class="text-gray-600">Dette Restante:</span>
                <span id="paiementEpargneMontantRestant" class="font-bold text-red-600"></span>
            </div>
            <div class="flex justify-between items-center border-t border-blue-200 pt-1 mt-1">
                <span class="text-gray-600">Solde Épargne Dispo:</span>
                <span id="paiementEpargneSolde" class="font-bold text-green-600"></span>
            </div>
        </div>

        <form id="payerDetteEpargneForm" method="POST" action="">
            <input type="hidden" id="detteIdPaiementEpargne" name="dette_id_paiement_epargne">

            <div class="form-group mb-6">
                <label for="montant_paiement_epargne" class="block text-gray-700 text-sm font-bold mb-2">Montant à
                    prélever:</label>
                <div class="relative">
                    <input type="number" step="0.01" id="montant_paiement_epargne" name="montant_paiement_epargne"
                        required min="0.01"
                ...
                <p class="text-xs text-gray-500 mt-1">Max: <span id="maxPaiementEpargneAmount"></span> XOF</p>
            </div>

            <button type="submit" name="payer_dette_epargne"
                ...
                <i class="fas fa-check-circle mr-2"></i> Valider le Paiement Épargne
            </button>
        </form>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // --- Gestion du Modal de Paiement via Épargne ---
        const payerDetteEpargneModal = document.getElementById('payerDetteEpargneModal');
        const paiementEpargneClientNomSpan = document.getElementById('paiementEpargneClientNom');
        const paiementEpargneMontantRestantSpan = document.getElementById('paiementEpargneMontantRestant');
        const paiementEpargneSoldeSpan = document.getElementById('paiementEpargneSolde');
        const detteIdPaiementEpargneInput = document.getElementById('detteIdPaiementEpargne');
        const montantPaiementEpargneInput = document.getElementById('montant_paiement_epargne');
        const maxPaiementEpargneAmountSpan = document.getElementById('maxPaiementEpargneAmount');

        document.querySelectorAll('.payer-dette-epargne-btn').forEach(button => {
            button.addEventListener('click', function () {
                const detteId = this.dataset.id;
                const clientNom = this.dataset.client_nom;
                const montantRestant = parseFloat(this.dataset.montant_restant);
                const soldeEpargne = parseFloat(this.dataset.solde_epargne);

                // Le montant max qu'on peut payer est le min entre ce qu'on doit et ce qu'on a
                const maxPayment = Math.min(montantRestant, soldeEpargne);

                paiementEpargneClientNomSpan.textContent = clientNom;
                paiementEpargneMontantRestantSpan.textContent = montantRestant.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' XOF';
                paiementEpargneSoldeSpan.textContent = soldeEpargne.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' XOF';

                detteIdPaiementEpargneInput.value = detteId;
                montantPaiementEpargneInput.value = maxPayment.toFixed(2);
                montantPaiementEpargneInput.max = maxPayment.toFixed(2);
                maxPaiementEpargneAmountSpan.textContent = maxPayment.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

                openModal(payerDetteEpargneModal);
            });
        });
        ...
        modalCloseButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                closeModal(payerDetteEpargneModal);
            });
        });
        ...
        window.addEventListener('click', function (event) {
            ...
            if (event.target == payerDetteEpargneModal) closeModal(payerDetteEpargneModal);
        });
