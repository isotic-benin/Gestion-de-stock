...
// Vérifier les permissions d'accès au module personnel (lecture)
if (!hasPermission($conn, 'personnel', 'read')) {
...
$can_create = hasPermission($conn, 'personnel', 'create');
$can_update = hasPermission($conn, 'personnel', 'update');
$can_delete = hasPermission($conn, 'personnel', 'delete');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['add_personnel']) || isset($_POST['edit_personnel']))) {
    ...
    if (isset($_POST['add_personnel']) && !$can_create) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission d\'ajouter un membre du personnel.</div>';
        goto skip_personnel_action;
    }
    if (isset($_POST['edit_personnel']) && !$can_update) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de modifier un membre du personnel.</div>';
        goto skip_personnel_action;
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_personnel'])) {
    ...
    if (!$can_delete) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de supprimer un membre du personnel.</div>';
        goto skip_delete_personnel;
    }
