...
// Vérifier les permissions d'accès au module ventes (création de ventes)
if (!hasPermission($conn, 'ventes', 'create')) {
...
<div id="saleConfirmModal"
    ...
    <script>
        ...
        function attachCartListeners() {
            // Utilisation de délégation d'événements ou attachement direct
            // Ici on garde l'attachement direct car renderCart recrée les éléments
            // Mais on va passer à 'input' pour la réactivité, car on ne re-render plus tout le tableau dans updateCartItem

            document.querySelectorAll('.quantity-input').forEach(input => {
                input.removeEventListener('input', updateCartItem); // Eviter doublons si appel multiple
                input.addEventListener('input', updateCartItem);
            });
            document.querySelectorAll('.unit-price-input').forEach(input => {
                input.removeEventListener('input', updateCartItem);
                input.addEventListener('input', updateCartItem);
            });
        ...
        function updateCartItem(event) {
            ...
            // Si la valeur est vide ou invalide pendant la saisie, on ne fait rien (on attend une valeur valide)
            if (isNaN(newValue) && event.target.value !== '') return;
            if (event.target.value === '') return;

            // Références aux inputs de la ligne pour mise à jour directe
            const row = event.target.closest('tr');
            const qtyInput = row.querySelector('.quantity-input');
            const priceInput = row.querySelector('.unit-price-input');
            const discountPercentInput = row.querySelector('.line-discount-percentage-input');
            const discountAmountInput = row.querySelector('.line-discount-amount-input');
            const lineTotalCell = row.querySelector('td:nth-child(6)'); // Cellule Total Ligne
            ...
            if (event.target.classList.contains('quantity-input')) {
                ...
                if (newValue < 0) { // On autorise 0 ou vide temporairement mais pas négatif
                    newValue = 1;
                    event.target.value = newValue.toFixed(2);
                }
                ...
                // Mise à jour visuelle du champ prix unitaire (si ce n'est pas lui qu'on édite)
                ...
                if (document.activeElement !== discountAmountInput) {
                    discountAmountInput.value = item.lineDiscountAmount.toFixed(2);
                }
                ...
                if (item.originalUnitPrice > 0) {
                    ...
                    if (item.stock && item.quantity > item.stock) {
                        ...
                        item.unitPrice = item.originalUnitPrice * item.quantity;
                        // On remet la valeur max dans le champ prix qu'on édite car on a plafonné
                        event.target.value = item.unitPrice.toFixed(2);
                        showMessage('warning', `Limité par le stock : ${item.stock}`);
                    }

                    // Mise à jour visuelle du champ quantité
                ...
                if (document.activeElement !== discountPercentInput) {
                    discountPercentInput.value = item.lineDiscountPercentage.toFixed(2);
                }
            }
            ...
            // S'assurer que les valeurs ne sont pas NaN pour le stockage
            ...
            // Mise à jour du texte Total Ligne
            lineTotalCell.textContent = item.lineTotal.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' ' + currency;

            // Note: On N'APPELLE PLUS renderCart() ici pour éviter de perdre le focus
            calculateTotals();
        }
