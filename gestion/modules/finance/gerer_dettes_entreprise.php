...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['add_dette_magasin']) || isset($_POST['edit_dette_magasin']))) {
    ...
    $montant_total = (float) sanitize_input($_POST['montant_initial']); // Le formulaire envoie encore 'montant_initial' comme name, on le mappe sur montant_total
    $date_dette = sanitize_input($_POST['date_emission']); // Mappé sur date_dette
    // Date Echeance et Statut ignorés car non présents en base
    ...
    $personnel_id_enregistrement = $_SESSION['id'];

    if (isset($_POST['add_dette_magasin'])) {
        // Ajout
        // Colonnes réelles : description, montant_total, montant_paye, date_dette, fournisseur_id, magasin_id, personnel_id_enregistrement
        $sql = "INSERT INTO dettes_magasins (description, montant_total, montant_paye, date_dette, fournisseur_id, magasin_id, personnel_id_enregistrement) VALUES (?, ?, 0, ?, ?, ?, ?)";
        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "sdsiis", $description, $montant_total, $date_dette, $fournisseur_id, $magasin_id, $personnel_id_enregistrement);
        ...
        $sql = "UPDATE dettes_magasins SET description = ?, montant_total = ?, date_dette = ?, fournisseur_id = ?, magasin_id = ? WHERE id = ?";
        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "sdsiii", $description, $montant_total, $date_dette, $fournisseur_id, $magasin_id, $dette_id);
            if (mysqli_stmt_execute($stmt)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['enregistrer_paiement'])) {
    ...
    // Récupérer le montant total et déjà payé
    $current_montant_restant = 0;
    $sql_current = "SELECT montant_total, montant_paye FROM dettes_magasins WHERE id = ?";
    if ($stmt_current = mysqli_prepare($conn, $sql_current)) {
        ...
        mysqli_stmt_bind_result($stmt_current, $total, $deja_paye);
        mysqli_stmt_fetch($stmt_current);
        mysqli_stmt_close($stmt_current);

        $current_montant_restant = $total - $deja_paye;
    } else {
        $message = '<div class="alert alert-error">Erreur de préparation : ' . mysqli_error($conn) . '</div>';
        goto end_paiement_process;
    }
    ...
    try {
        // 1. Mettre à jour le montant payé dans dettes_magasins
        ...
        // Colonnes : id, dette_magasin_id, montant_paiement, date_paiement, personnel_id
        $sql_insert_paiement = "INSERT INTO paiements_dettes_magasins (dette_magasin_id, montant_paiement, personnel_id) VALUES (?, ?, ?)";
        if ($stmt_insert_paiement = mysqli_prepare($conn, $sql_insert_paiement)) {
            mysqli_stmt_bind_param($stmt_insert_paiement, "idi", $dette_id, $montant_paye, $personnel_id);
...
$sort_by = isset($_GET['sort_by']) ? sanitize_input($_GET['sort_by']) : 'dm.date_dette';
                                ...
                                <?php if (!empty($dettes_magasins)): ?>
                                    <?php foreach ($dettes_magasins as $dette): ?>
                                        <tr>
                                            ...
                                            <td><?php echo number_format($dette['montant_total'], 2, ',', ' '); ?> XOF</td>
                                            <td
                                            ...
                                            <td><?php echo !empty($dette['date_echeance']) ? date('d/m/Y', strtotime($dette['date_echeance'])) : 'N/A'; ?>
                                            </td>
                                            ...
                                            <td class="action-buttons flex-wrap">
                                                ...
                                                <?php if ($can_update): ?>
                                                    <button class="btn btn-edit edit-dette-magasin-btn"
                                                        ...
                                                        data-description="<?php echo htmlspecialchars($dette['description'] ?? ''); ?>"
                                                        data-montant_initial="<?php echo htmlspecialchars($dette['montant_total']); ?>"
                                                ...
                                                <?php endif; ?>
                                                <?php if ($can_update && $dette['statut'] != 'payee' && $dette['statut'] != 'annulee'): ?>
