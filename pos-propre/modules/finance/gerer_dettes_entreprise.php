...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['enregistrer_paiement'])) {
    ...
    try {
        ...
        $sql_update_dette = "UPDATE dettes_magasins SET montant_restant = ?, statut = ? WHERE id = ?";
        if ($stmt_update_dette = mysqli_prepare($conn, $sql_update_dette)) {
            mysqli_stmt_bind_param($stmt_update_dette, "dsi", $new_montant_restant, $new_statut, $dette_id);
...
$sort_by = isset($_GET['sort_by']) ? sanitize_input($_GET['sort_by']) : 'dm.date_emission';
...
$sql = "SELECT dm.id, dm.description, dm.montant_initial, dm.montant_restant, dm.date_emission, dm.date_echeance, dm.statut, dm.date_enregistrement,
