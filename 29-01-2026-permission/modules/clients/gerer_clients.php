...
if (!hasPermission($conn, 'clients', 'read')) {
...
$can_create = hasPermission($conn, 'clients', 'create');
$can_update = hasPermission($conn, 'clients', 'update');
$can_delete = hasPermission($conn, 'clients', 'delete');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['add_client']) || isset($_POST['edit_client']))) {
    ...
    if (isset($_POST['add_client']) && !$can_create) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission d\'ajouter des clients.</div>';
        goto end_form_processing_clients;
    }
    if (isset($_POST['edit_client']) && !$can_update) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de modifier des clients.</div>';
        goto end_form_processing_clients;
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_client'])) {
    ...
    if (!$can_delete) {
        $message = '<div class="alert alert-error">Vous n\'avez pas la permission de supprimer des clients.</div>';
    } else {
